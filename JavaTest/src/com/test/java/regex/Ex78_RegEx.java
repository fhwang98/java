...
public class Ex78_RegEx {
	...
	public static void main(String[] args) {
		// 정규표현식 + 자바 > 적용
		//- 검색 (+치환) > replaceAll(), replaceFiirst()
		//- 분리 > split()
...
;
		...
		regex = "^[0-9]{2,3}$"; //정규표현식으로는 정확한 수의 범위 비교 할 수 없음
		p1 = Pattern.compile(regex);
		m1 = p1.matcher(age);
		if (!m1.find() || (Integer.parseInt(age) < 18 || (Integer.parseInt(age) > 100))) {
			System.out.println("나이 > 필수, 숫자, 18세 이상");
			return false;
		}
	...
	private static void m3() {
		...
		//System.out.println(m1.find()); //검색 결과 (boolean)
...
//			count++;
//		}
//		System.out.printf("전화번호가 총 %d회 발견됐습니다.\n", count);
...
//		System.out.println(txt.replaceAll("\\w+@[a-zA-Z_]+?\\.[a-zA-Z]{2,3}", "XXXX@XXXX"));
		System.out.println(
				txt
				.replaceAll("010-[0-9]{3,4}-[0-9]{4}", "XXX-XXXX-XXXX")
				.replaceAll("\\w+@[a-zA-Z_]+?\\.[a-zA-Z]{2,3}", "XXXX@XXXX"));
		System.out.println();
